    ...
    for track in tracker.tracks:
        ...
        if save_txt:  # Write to file
            with open(txt_path + '.txt', 'a') as f:
                f.write('frame: {}; track: {}; class: {}; xmin: {:.0f}, ymin:{:.0f}, xmax:{:.0f}, ymax:{:.0f}:;\n'.format(frame_count, track.track_id, class_num,
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        video_frame_rate = vid_cap.get(cv2.CAP_PROP_FPS)  # Replace YOUR_VIDEO_FRAME_RATE_HERE with the actual frame rate
        frames_per_second = video_frame_rate
                ...
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        vid_writer = cv2.VideoWriter(
                            save_path, cv2.VideoWriter_fourcc(*fourcc), fps_1, (w, h))
